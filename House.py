...
#ub_lb_range = upper_bound - lower_bound

class ACUnit:

    def __init__(self, power, setpoint, range, t_initial):
...
class House:
    ...
    def __init__(self, A_wall, A_window, A_floor, U_wall, U_window, U_floor,
                 height, cooling_cap, heating_cap, shgc, perc_s_windows, people, v_rate, setpoint_ac, setpoint_hp,
                 shade_factor=0.7, t_initial = 292, t_des=295):
    ...
    def set_inertia(self): #[J/K]
        ...
        cc_term = cp_cc * self.A_wall * wall_th
        return air_term + cc_term

    def get_heatgain(self, t_out, t_in, solar_irr): # [W], heat gain per timestep
        heat_gain = (self.A_wall * self.U_wall + self.A_window * self.U_window + self.A_floor * self.U_floor + self.v_rate/3600 * self.A_floor) * (t_out - t_in)
...
class RunSimulation:
    ...
    def run_scenario(self):
        ...
        for idx,year in enumerate(years):
            self.temp_list_kelvin = [t+increase_temp[idx] for t in self.temp_list_kelvin] #absolute increase in temperature
    ...
    def run(self):
        ...
        for idx, t in enumerate(self.temp_list_kelvin):
            t_next = t_list[idx] + self.house.get_t_diff(self.timestep, t, t_list[idx], self.irr_list[idx])
        ...
        tot_demand_AC = sum(e_demand_list_AC)/cop
...
class Plot_output:
    ...
    def plot_temperature(self):
        for year in self.output:
            plt.plot(self.days_in_year, self.output[year][0], label='SFH after 2000 '+str(year))
    ...
    def plot_ac_demand(self):
        for year in self.output:
            plt.plot(self.days_in_year, self.output[year][1], label='SFH after 2000 ' + str(year))
            # plt.plot(days_in_year, T_SFH_before, label='SFH before 2000')
